...
critical_density = 1.306 #obtained from https://doi.org/10.1073/pnas.1619260114
densities = [1.29,1.293,1.295,1.298,1.30,1.303,critical_density]
volume = size**2 #for 2D model

rmax = 4500 #max radius of density sampling window
radii = np.linspace(1,rmax,5000) #radii of spherical windows used to compute density fluctuations
...
for j,rho in enumerate(densities):
    ...
    for k,l in enumerate(radii):
        fluctuations[j,k], *_ = DensityFluctuations(grid,grid_points,grid_tree,size,l,100)

np.save('dens_flucs_2d.npy', fluctuations)
np.save('radii_2d.npy',radii)
