...
def DensityFluctuations(grid,L,l,sample_size,sampling_type='continuous',perf_check=True):
    ...
    if sampling_type not in ['continuous','discrete']:
        ...
        for k in range(sample_size):
            ...
            imin, jmin = np.ceil(center - l).astype(int)
            imax, jmax = np.ceil(center + l).astype(int)
            ishift = 0
            jshift = 0
            ...
            if np.any([ishift,jshift]):
                grid_to_sample = np.roll(grid_to_sample,ishift,axis=0) 
                grid_to_sample = np.roll(grid_to_sample,jshift,axis=1)

            sampled_grid = grid_to_sample[imin:imax,jmin:jmax]
...
np.save('fluctuations_data_copy.npy',fluctuations_data)
np.save('avg_times_data_copy.npy',avg_time_data)
np.save('var_times_data_copy.npy',var_time_data)
